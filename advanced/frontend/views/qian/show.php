...
//AppAsset::addScript($this,'/assets/js/jquery.js'); 
//在head区加载js 
 $this->registerJsFile('@web/assets/js/jquery.js',['depends'=>['frontend\assets\AppAsset']]);

$this->registerCssFile('assets/css/index.css',['depends'=>['frontend\assets\AppAsset']]); 
?>
...
$(function(){
                ...
                function showDate(yyyy,mm){
                    ...
                    $.ajax({
                        url:'http://localhost/nine/advanced/frontend/web/index.php?r=qian/show1',
                        dataType:'json',
...
$(document).on('click','.f-day',function(){
    ...
    if(logday>newday){
        ...
        if($(this).parent().find("#aa").attr('class')!= ''){
            ...
            if(con == true){
                $.ajax({
                    url:'http://localhost/nine/advanced/frontend/web/index.php?r=qian/bu',
                    data:{logday:logday},
                    type:'get',
                    dataType:'json',
                    success:function(msg){
        ...
        if($(this).parent().find("#aa").attr('class')!= ''){
            ...
            $.ajax({
            url:'http://localhost/nine/advanced/frontend/web/index.php?r=qian/fen',
            data:{newday:newday},
            type:'get',
            dataType:'json',
            success:function(msg){
    ...
    if(date[2]==daysCount){
        $.ajax({
            url:'http://localhost/nine/advanced/frontend/web/index.php?r=qian/di',
